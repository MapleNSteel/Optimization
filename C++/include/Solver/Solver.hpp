...
public:
   ...
   */
  virtual const Eigen::Matrix<T, NX+NH+NG, 1> solve(const Eigen::Matrix<T, NX, 1>& candidate_vector, const std::optional<Eigen::Matrix<T, NH, 1>>& sigma_vector = std::nullopt, const std::optional<Eigen::Matrix<T, NG, 1>>& lambda_vector = std::nullopt) const = 0;
protected:
