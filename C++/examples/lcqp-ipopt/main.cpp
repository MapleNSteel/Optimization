...
int main() {
    ...
    Eigen::Matrix<double, NG, 1> lambda = mu*lcqp_1.inequalityConstraintVector(x0).value().cwiseInverse();

    for (size_t num = 0; num < NUM_ITER; num++) {
        const Eigen::Matrix<double, NX+NG, 1> x_opt = ipopt.solve(x0, lambda);

        x0.block<NX, 1>(0, 0) = x_opt.block<NX, 1>(0, 0).eval();
        lambda.block<NG, 1>(0, 0) = x_opt.block<NG, 1>(NX, 0).eval();
